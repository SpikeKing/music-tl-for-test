...
from data_preprocessor import check_error_features
from utils import time_2_readable, traverse_dir_files
...
def get_label_dict(path_list):
    ...
    根据音频路径列表，生成标签字典
    """
    normal_num = 4  # 音频的正常样本数量，小于即认为异常样本
...
def mp_data_merger(data_dir, out_dir, n_prc=40):
    ...
    for path in path_list:
        ...
        if clz_name not in label_dict:
            ...
            continue
        results.append(p.apply_async(prc_func, args=(path, clz_name,)))
...
def merge_data():
    ...
    """
    npy_path = os.path.join(ROOT_DIR, 'data', 'npy_data')
    npz_out = os.path.join(ROOT_DIR, "data", "data_all.npz")
    # mp_data_merger(data_dir=test_path, out_dir=test_out)
    mp_data_merger(npy_path, npz_out)
